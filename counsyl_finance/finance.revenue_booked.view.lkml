...
view: booked_revenue {
  derived_table: {
    sql: SELECT
        ...
        , revenue
        , transid
        , order_id::text || rev_rec_date::text || revenue_type || revenue::text || transid::text AS pkey -- check to see if this is unique
      FROM
        uploads.barcoderevenue_w_transid
       ;;
    sql_trigger_value: select sum(revenue) from uploads.barcoderevenue_w_transid ;;
  ...
  dimension: booked_aggregate_type {
    ...
    sql: CASE
          WHEN ${TABLE}.revenue_type IN ('LBM Adjustment','OON Payer True Up','OON Patient True Up','Insight Revenue Adjustment','Additional Direct Bill','Kaiser Adjustment','Medicare Adjustment','Microdeletions Adjustment','Self Pay True Up','INN Payer True Up','INN Patient True Up') THEN ${TABLE}.revenue_type
...
view: booked_revenue_order_level {
      ...
      LEFT JOIN
        (SELECT
          ...
          , SUM(CASE
                ...
                OR revenue_type = 'Microdeletions Adjustment'
                OR revenue_type = 'LBM Adjustment'
                OR revenue_type = 'Medicare Adjustment'
                OR revenue_type = 'INN Charge Cap Adjustment'
                OR revenue_type = 'OON Charge Cap Adjustment'
                OR revenue_type = 'Kaiser Adjustment'
          ...
          , SUM(CASE
                WHEN revenue_type = 'Self Pay True Up'
                  THEN revenue ELSE null END) AS self_trueup
          , SUM(CASE
          ...
          , SUM(CASE
                ...
                OR revenue_type = 'Self Pay True Up'
          ...
          , SUM(CASE
                ...
                OR revenue_type = 'JScreen Consignment'
                OR revenue_type = 'Insurer Non-Payment'
                  THEN revenue ELSE null END) AS recurring_revenue
          , SUM(CASE
        ...
        GROUP BY 1) as sub on sub.order_id = o.id
  ...
  dimension: recurring_revenue {
    hidden: yes
    description: "The dollar amount in revenue attributed to recurring revenue that was booked by acccounting, i.e. - In-Network accrued revenue, OON accrued revenue, OON cash-basis payments, non-payments, In-Network accrued customer revenue, OON accrued customer revenue, OON cash-basis customer payments, MOOP revenue, invoice flux, consignment revenue, consighment comps (-), JScreen revenue and credit card revenue"
    type: number
    sql: ${TABLE}.recurring_revenue ;;
  }
  ...
  measure: total_recurring_revenue {
    group_label: "Sub-Totals A"
    hidden: yes
    label: "$ Total Booked Recurring Revenue"
    description: "The total dollar amount in revenue attributed to recurring revenue that was booked by acccounting, i.e. - In-Network accrued revenue, OON accrued revenue, OON cash-basis payments, non-payments, In-Network accrued customer revenue, OON accrued customer revenue, OON cash-basis customer payments, MOOP revenue, invoice flux, consignment revenue, consighment comps (-), JScreen revenue and credit card revenue"
    value_format_name: usd
    type: sum
  ...
  measure: average_recurring_revenue {
    group_label: "Sub-Totals A"
    hidden: yes
    label: "$ Average Total Booked Recurring Revenue"
    description: "The average dollar amount in revenue attributed to recurring revenue that was booked by acccounting, i.e. - In-Network accrued revenue, OON accrued revenue, OON cash-basis payments, non-payments, In-Network accrued customer revenue, OON accrued customer revenue, OON cash-basis customer payments, MOOP revenue, invoice flux, consignment revenue, consighment comps (-), JScreen revenue and credit card revenue"
    value_format_name: usd
    type: number
    sql: ${total_recurring_revenue} / COUNT(${order_id}) ;;
  }
