view: all_revenue {
  derived_table: {
    sql: -- ===================== START ======================================================================================================
                  ...
                  SELECT
                    consignment_revenue.order_id AS order_id
                    , consignment_revenue.barcode AS barcode
                    , consignment_revenue.revenue_type AS revenue_type
                    , 'Consignment' AS aggregate_type
                    , consignment_revenue.revenue_recorded_timestamp AS revenue_recorded_timestamp
                    , consignment_revenue.pkey_type
                    , consignment_revenue.pkey
                    , product
                    , SUM(coalesce(consignment_revenue.revenue,0)) AS revenue
                  FROM ${consignment_revenue.SQL_TABLE_NAME} AS consignment_revenue
      ...
      sql_trigger_value: select string_agg(to_char(full_count,'99999999'),',') from ((select count(*) AS full_count from ${consignment_revenue.SQL_TABLE_NAME}) UNION (select count(*) AS full_count from ${revenue_inn_barcode.SQL_TABLE_NAME})) AS foo ;;
