...
view: revenue_oon_match_all_fields {
  derived_table: {
    sql:
      ...
      INNER JOIN
        uploads.payment_lag_factor_w_oon as lag on lag.month = o.completed_month
...
view: revenue_oon_lookback_base_moop {
  derived_table: {
    sql: SELECT
      ...
      GROUP BY 1, 2, 3, 4
       ;;
    sql_trigger_value: select sum(trigger) from ( select sum(order_count) as trigger from ${revenue_oon_target_moop.SQL_TABLE_NAME} union all select sum(hybrid_transaction_amt) as trigger from ${matched_report_2014_to_current.SQL_TABLE_NAME}) as t ;;
  }
...
view: revenue_oon {
  derived_table: {
    sql: SELECT
              ...
              , SUM(CASE
                 WHEN rb.full_match_order_count <> 0 THEN rb.full_match_payer_revenue
                 WHEN rb.ppldmp_order_count <> 0 THEN rb.ppldmp_payer_revenue
                 WHEN rb.ppdmp_order_count <> 0 THEN rb.ppdmp_payer_revenue
                 WHEN rb.pdpmp_order_count <> 0 THEN rb.pdpmp_payer_revenue
                 WHEN rb.pdp_order_count <> 0 THEN rb.pdp_payer_revenue
                 WHEN rb.pp_order_count <> 0 THEN rb.pp_payer_revenue
                 WHEN rb.dpp_order_count <> 0 THEN rb.dpp_payer_revenue
                 WHEN rb.po_order_count <> 0 THEN rb.po_payer_revenue ELSE 0 END)
              / (nullif(sum(CASE
                 WHEN rb.full_match_order_count <> 0 THEN rb.full_match_total_charges
                 WHEN rb.ppldmp_order_count <> 0 THEN rb.ppldmp_total_charges
                 WHEN rb.ppdmp_order_count <> 0 THEN rb.ppdmp_total_charges
                 WHEN rb.pdpmp_order_count <> 0 THEN rb.pdpmp_total_charges
                 WHEN rb.pdp_order_count <> 0 THEN rb.pdp_total_charges
                 WHEN rb.pp_order_count <> 0 THEN rb.pp_total_charges
                 WHEN rb.dpp_order_count <> 0 THEN rb.dpp_total_charges
                 WHEN rb.po_order_count <> 0 THEN rb.po_total_charges ELSE 0 END),0)) as revenue_percent
              , SUM(CASE
      ...
      FROM
        ...
        WHERE
          (CASE
            ...
            ELSE 'OON'
            END) = 'OON'
          AND o.completed_on >= '2017-01-01'  -- NEED TO UPDATE TO CORRECT EFFECTIVE DATE ONCE WE DECIDE TO GO LIVE
    ...
    sql:
        SELECT
          ...
          , CASE
            ...
            WHEN status_name = 'Canceled Chose Consignment' THEN 'OON Patient INCL MOOP'
            WHEN status_name = 'Maximum OOP - No Insurance' THEN 'OON Patient INCL MOOP'
            WHEN status_name = 'Canceled' THEN 'OON Patient INCL MOOP'
    ...
    sql:
          ...
          FROM
            ${oon_invoice_flux_patient_invoices.SQL_TABLE_NAME} AS invoice_flux_patient_invoices
              ...
              GROUP BY 1,2,3,4,5,6
           ;;
    sql_trigger_value: select count(*) from ${oon_invoice_flux_patient_invoices.SQL_TABLE_NAME} ;;
...
view: oon_flux_factor_applied_to_barcodes {
  derived_table: {
    sql: SELECT
      foo2.order_id
      , foo2.latest_barcode AS barcode
      , foo2.status_name
      , foo2.order_status_name
      , foo2.product
      , foo2.completed_on
      , foo2.revenue_amount
      , foo2.payer_id
      ...
      , coalesce(foo2.revenue_amount*invoice_flux_factor.invoice_flux_factor,foo2.revenue_amount) AS revenue_amount_adjusted_for_flux
      , (foo2.revenue_amount*invoice_flux_factor.invoice_flux_factor) - foo2.revenue_amount AS invoice_flux_revenue
      FROM
        (
          SELECT
            latest_barcode
          ...
          FROM
            (
              SELECT
                ord.latest_barcode
                ,ord.id AS order_id
                ,TO_CHAR(ord.completed_on, 'YYYY-MM') AS completed_month
                ,ord.completed_on
              ...
              INNER JOIN ${order.SQL_TABLE_NAME} AS ord ON ord.latest_barcode::text = revenue_oon_barcode.latest_barcode::text
          ...
          INNER JOIN
            ${oon_flux_claim_state.SQL_TABLE_NAME} AS flux_claim_state ON flux_claim_state.status_name = foo.status_name
        ) AS foo2
      INNER JOIN ${oon_invoice_flux_factor.SQL_TABLE_NAME} AS invoice_flux_factor
        ON invoice_flux_factor.product = foo2.product
        AND invoice_flux_factor.completed_month  = foo2.completed_month
        AND invoice_flux_factor.flux_claim_state = foo2.flux_claim_state
       ;;
...
#== This model simply takes orders marked as self-pay and applies 98% of the invoiced amount as the revenue recognized ==#
  ...
  derived_table: {
    sql:
      SELECT
        ...
        , (inv.amount * .98) AS self_pay_accrual -- note the .98 is the % of invoiced charges we are recognizing for these orders.
